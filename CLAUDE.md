...
`main.py` (5 lines) → `game/app.py:create_game()` — Initializes pygame, creates Game instance, runs menu scene then gameplay scene.

### Engine (`engine/`) — Reusable across projects
...
### Game (`game/`) — Specific to The Quest-ionnable Heroe
...
- `game/features/character/data/classes.json` — 6 player classes (Warrior, Mage, etc.)
- `game/features/character/data/enemies.json` — 4 enemy types
- `game/features/character/data/npcs.json` — NPC definitions
- `game/features/world_map/data/maps.json` — Map configs + teleporters
- `game/features/world_map/data/spawns.json` — Enemy/NPC spawn definitions
- `game/assets/manifest.json` — Asset file paths (replaces 1400-line images.py)
...
grep -rl "import pygame" engine/core/ game/features/*/components.py game/features/*/logic.py
